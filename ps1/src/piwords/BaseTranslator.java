...
public class BaseTranslator {
    ...
    public static int[] convertBase(int[] digits, int baseA,
    	...
    	if( baseA < 2 || baseB <2 || precisionB < 1 || digits.length == 0){
    	...
    	while(i < precisionB){
    		...
    		for ( int k = digiCopy.length - 1; k >= 0; k-- ) {
    			
    			if ( digiCopy[k] < 0 || digiCopy[k] >= baseA ) {
    				
    				return null;
    				
    			}

    			x = (digiCopy[k]*baseB) + carry;
